/*****************************************************************
 * 						Generated by Xpand Eclipse
 *						File :	Os_Cfg.h
 *						Date : 
 *						Author : Matthieu Kowalewski
 *						License : GPLv2
 ****************************************************************/
#ifndef _XENOMAI_AUTOSAR_OS_CFG_H
#define _XENOMAI_AUTOSAR_OS_CFG_H
/*********************** AppMode ***********************/
#define app1_ID			1u	

#define app1				(AppModeType)app1_ID	

/*********************** Resource ***********************/
#define OsResource1_ID			1u	
#define OsResource2_ID			2u	
#define OsResource3_ID			3u	
#define OsResource4_ID			4u	

#define OsResource1				(ResourceType)OsResource1_ID	
#define OsResource2				(ResourceType)OsResource2_ID	
#define OsResource3				(ResourceType)OsResource3_ID	
#define OsResource4				(ResourceType)OsResource4_ID	

/************************ Tasks ************************/
#define OsTask3_ID			1u	
#define OsTask2_ID			2u	
#define OsTask1_ID			3u	

#define OsTask3				(TaskType)OsTask3_ID	
#define OsTask2				(TaskType)OsTask2_ID	
#define OsTask1				(TaskType)OsTask1_ID	

/********************** Counters **********************/
#define counter1_ID			1u	

#define counter1				(CounterType)counter1_ID	

/*********************** Alarms ***********************/
#define alarm1_ID			1u	

#define alarm1				(AlarmType)alarm1_ID	

/*********************** RASPBERRY GPIO ***********************/

#define GPIO4           1
#define GPIO5           2
#define GPIO6           3
#define GPIO12          4
#define GPIO13          5
#define GPIO16          6
#define GPIO17          7
#define GPIO19          8
#define GPIO20          9
#define GPIO21          10
#define GPIO22          11
#define GPIO23          12
#define GPIO24          13
#define GPIO25          14
#define GPIO26          15
#define GPIO27          16

#endif //_XENOMAI_AUTOSAR_OS_CFG_H
